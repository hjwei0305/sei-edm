        ...
        <div id="collapseTwo" class="panel-collapse collapse">
            <div class="panel-body">
                ...
                <div style="padding: 10px">
                    <form enctype="multipart/form-data" id="fileUploadFS">
                        <input type="file" name="fileFS"/>
                        <input type="button" id="btnsubmitFS" value=" 上 传 "/>
                    </form>
                    <form id="clearCache">
                        <input type="button" id="btnresetFS" value=" 清 空 "/>
                    </form>
                </div>
                <div>
                    <table id="tableFS" data-pagination="true"></table>
                </div>
            </div>
        </div>
    </div>
        ...
        <div id="collapse3" class="panel-collapse collapse in">
            <div class="panel-body">
                <p style="color: red;">
                    ...
                    预览：/preview GET  ${baseUrl}preview/{docId}</br>
    ...
    <div class="panel panel-default">
        ...
        <div id="collapse4" class="panel-collapse collapse">
            <div class="panel-body">
                <p style="color: red;">
                    地址示例：</br>
                    电子发票二维码识别：/ocr/invoice POST 参数 <strong>file</strong> ${baseUrl}ocr/invoice </br>
                    图片/PDF文件条码或二维码识别：/ocr/upload POST 参数 <strong>file</strong> ${baseUrl}ocr/upload
                </p>
                <div style="padding: 10px">
                    <form enctype="multipart/form-data" id="fileUploadOcr1">
                        <input type="file" name="file"/>
                        <input type="button" id="ocrInvoice" value=" 电子发票 "/>
                    </form>
                    <hr>
                    <form enctype="multipart/form-data" id="fileUploadBarcode">
                        <input type="file" name="file"/>
                        <input type="button" id="ocrBarcode" value=" 条码/二维码 "/>
                    </form>
                <#--<form id="clearCache">-->
                <#--<input type="button" id="btnreset" value=" 清 空 " />-->
                <#--</form>-->
...
<script>
    ...
    $(function () {
        ...
        $("#btnsubmit").click(function () {
            ...
            $("#fileUpload").ajaxSubmit({
                success: function (data) {
                    ...
                    html = "<a target='_blank' href='${baseUrl}preview/" + data[0]['id'] + "'>预览" + data[0]['fileName'] + "</a></br>";
        ...
        //----------------------ocrInvoice---------------------
        $("#ocrInvoice").click(function () {
            showLoadingDiv();
            $("#fileUploadOcr1").ajaxSubmit({
                success: function (data) {
                    // 上传完成，刷新table
                    console.log(JSON.stringify(data));
                    var html = '';
                    html += "<table>";
                    html += "<tr><th>文档ID: </th><td>"+data["docId"]+"</td></tr>";
                    html += "<tr><th>文件名称: </th><td>"+data["fileName"]+"</td></tr>";
                    html += "<tr><th>发票代码: </th><td>"+data["invoiceCode"]+"</td></tr>";
                    html += "<tr><th>发票号码: </th><td>"+data["invoiceNumber"]+"</td></tr>";
                    html += "<tr><th>发票种类代码: </th><td>"+data["categoryRemark"]+"</td></tr>";
                    html += "<tr><th>开票金额: </th><td>"+data["amount"]+"</td></tr>";
                    html += "<tr><th>开票日期: </th><td>"+data["invoiceDate"]+"</td></tr>";
                    html += "</table>";
                    $('#ocrData').html(html);
                ...
                url: 'ocr/invoice', /*设置post提交到的页面*/
                type: "post", /*设置表单以post方法提交*/
                dataType: "json" /*设置返回值类型为文本*/
            });
        });

        $("#ocrBarcode").click(function () {
            showLoadingDiv();
            $("#fileUploadBarcode").ajaxSubmit({
                success: function (ocrData) {
                    // 上传完成，刷新table
                    console.log(JSON.stringify(ocrData));
                    var data = ocrData[0];
                    var html = '';
                    html += "<table>";
                    html += "<tr><th>文件名: </th><td>"+data["fileName"]+"</td></tr>";
                    html += "<tr><th>识别内容: </th><td>"+data["ocrData"]+"</td></tr>";
                    html += "</table>";
                    $('#ocrData').html(html);
